...
#### 1.B.2.R Revision (Reviewer 1 comment 1)
...
```{r enc  correl interdiff phase}
# create reduced data.frame
intdiff_phase <- merge(data_bins_enc[,c(1,53,59)], cbind(score[,c(6,7)], stai$stai_sum, kubios$rmssd), by = "vp")
intdiff_phase$vp <- factor(intdiff_phase$vp)

# analyze (relevant) correlations
corr_intdiff_phase <- rcorr(as.matrix(intdiff_phase[-1]))
corr_intdiff_phase$r[1:2,3:5]
corr_intdiff_phase$P[1:2,3:5]

# nicht-signifikanter Trend von STAI mit systolic ratio
cor.test(intdiff_phase$click_lvet_rel, intdiff_phase$`stai$stai_sum`)
